...
class WithholdingTaxMove(models.Model):
    ...
    def generate_account_move(self):
        ...
        for line in move.line_ids:
            if line.account_id.user_type_id.type in ['payable', 'receivable'] and line.partner_id:
                line_to_reconcile = line
                break
        aml_recs = line_to_reconcile
        aml_recs |= self.credit_debit_line_id
        aml_recs.reconcile()
        ...
        # wizard = self.env['account.move.line.reconcile.writeoff'].with_context(
        #     active_ids=[x.id for x in aml_recs]).create(
        #     {'journal_id': self.bank_journal_usd.id, 'writeoff_acc_id': self.account_rsa.id})
        # wizard.trans_rec_reconcile()

        # self.credit_debit_line_id.move_id.post()
        # self.credit_debit_line_id.invoice_id.register_payment(line_to_reconcile)
