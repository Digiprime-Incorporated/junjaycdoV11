...
class PaymentInvoiceLine(models.Model):
    ...
    amount_total = fields.Float('Amount Total', digits=dp.get_precision('Account'))
    residual = fields.Float('Amount Due', digits=dp.get_precision('Account'))
    amount = fields.Float('Amount To Pay', digits=dp.get_precision('Account'),
    ...
    untaxed_amount = fields.Float('Untaxed Amount', digits=dp.get_precision('Account'))
    amount_tax = fields.Float('Tax', digits=dp.get_precision('Account'))
    check_amount = fields.Float('Check Amount', digits=dp.get_precision('Account'),
                                compute="compute_check_amount")
    wt_rate = fields.Float('WT', digits=dp.get_precision('Account'),
                           compute="compute_wt_rate")
    ...
    def onchange_select(self):
        if self.invoice_id.residual and self.invoice_id.amount_total:
            if self.select:
                #coeff_net = self.invoice_id.residual / self.invoice_id.amount_total
...
class AccountPayment(models.Model):
    ...
    invoice_total = fields.Float('Invoice Total', digits=dp.get_precision('Account'),
    ...
    selected_inv_total = fields.Float(compute='compute_selected_invoice_total',
        store=True, string='Assigned Amount', digits=dp.get_precision('Account'))
    balance = fields.Float(compute='_compute_balance', string='Balance')
    manual_selection = fields.Boolean('Manual Selection?', digits=dp.get_precision('Account'),
