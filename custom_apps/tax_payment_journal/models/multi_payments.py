...
class AccountRegisterPayments(models.TransientModel):
    ...
    tax_amount = fields.Float('Withholding Tax Amount',compute="multi_tax_calculate",digits=dp.get_precision('Product Price'))
    
    tax_type = fields.Char('Tax categ',compute="get_multitax_type")
    
    check_amount = fields.Float(string='Check Amount', digits=dp.get_precision('Product Price'),
                                compute="get_multi_check_amount")
    
    check_amount_in_word = fields.Char(string="Check Amount(in words)", compute="get_multi_check_amount_in_word")
                ...
                if total_amount > 0:
                    if invoice.residual < total_amount:
                        if invoice.amount_tax:
                            tax_amount +=((invoice.residual/1.12) * vat)
                        else:
                            tax_amount += (invoice.residual * vat)
    ...
    def create_payments(self):
        ...
        for payment_vals in self.get_payments_vals():
            ...
            if payment_vals.get('invoice_ids'):
                ...
                acc_invoice = self.env['account.invoice'].search([('id','in',invoice_ids)])
