...
class Stock_Move(models.Model):
    ...
    def _action_cancel(self):
        ...
        for eachmove in self.filtered(lambda l: l.state == 'done'):
            ...
            # update the delivered qty into the sale order line.
            if 'sale_line_id' in move_fields:
                if eachmove.sale_line_id:
                    eachmove.sale_line_id.qty_delivered = eachmove.sale_line_id._get_delivered_qty()
            if eachmove.account_move_ids:
                eachmove.account_move_ids.button_cancel()
                eachmove.account_move_ids.unlink()
